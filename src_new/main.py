...
def main():
    ...
    for ep in range(epochs):
        ...
        if ep % validation_freq == 0 or ep == epochs - 1: 
            if args.baseline:
                val_loss, val_score = evaluate2(model, args.device, val_loader, loss, metric, n_val, decoder, transform, fsd_epoch=ep if args.dataset == 'FSD' else None)
        ...
        if not args.baseline and ((ep + 1) % retrain_freq == 0 or ep == epochs - 1):
            ...
            if prev_param_values is not None and torch.equal(param_values, prev_param_values):
                ...
                if os.path.isfile(os.path.join(args.save_dir, 'network_hps.npy')):
                    ...
                    for lr, drop_rate, weight_decay, momentum in hp_configs:
                        ...
                        retrain_val_loss, retrain_val_score = evaluate2(retrain_model, args.device, search_val_loader, loss, metric, search_n_val, decoder, transform, fsd_epoch=200 if args.dataset == 'FSD' else None)
                ...
                get_params_to_update(retrain_model)
                ...
                for retrain_ep in range(retrain_epochs):
                    ...
                    if retrain_ep % validation_freq == 0 or retrain_ep == retrain_epochs - 1:
                        retrain_val_loss, retrain_val_score = evaluate2(retrain_model, args.device, retrain_val_loader, loss, metric, retrain_n_val, decoder, transform, fsd_epoch=retrain_ep if args.dataset == 'FSD' else None)
            ...
            test_loss, test_score = evaluate2(test_model, args.device, retrain_test_loader, loss, metric, retrain_n_test, decoder, transform, fsd_epoch=200 if args.dataset == 'FSD' else None)
            ...
            np.save(os.path.join(args.save_dir, 'dash_final_results.npy'), {'params':count_params(test_model),'ks':ks,'ds':ds,'lr':lr,'drop out':drop_rate,'weight decay':weight_decay,'momentum':momentum,'test best score':test_score})
...
def evaluate2(model, device, loader, loss, metric, n_eval, decoder=None, transform=None, fsd_epoch=None): #
    model.eval()
